...
namespace DBUtility
{
    ...
    public class DbHelperSQLP
    {
        ...
        {
            connectionString = PubConstant.GetConnectionString(ConnectionString);
        }
        ...
        /// <param name="tableName">表名称</param>
        /// <returns></returns>
        public DataTable GetAllColumnInfo(string tableName)
        {
            string sql = @"select c.name,c.id,c.xtype,c.xusertype,t.name as 'xtypeName',c.[length],c.colid,c.[cdefault] from syscolumns c left join sysTypes t on c.xtype=t.xtype and c.xusertype=t.xusertype where [id]=object_id(@tableName)";
            ...
            {
                throw new NoNullAllowedException("不存在的表" + tableName);
            }
            return ds.Tables[0];
        }
